...
struct metal_io_ops {
	...
	void*		(*offset_to_virt)(struct metal_io_region *io,
				 unsigned long offset);
	unsigned long	(*virt_to_offset)(struct metal_io_region *io,
					 void* virt);
...
{
	if (!io->ops.offset_to_virt) {
		return (io->virt != METAL_BAD_VA && offset < io->size
				? (uint8_t *)io->virt + offset
				: NULL);
	}

	return (*io->ops.offset_to_virt)(io, offset);
}
...
{
	if (!io->ops.virt_to_offset) {
		size_t offset = (uint8_t *)virt - (uint8_t *)io->virt;
		return (offset < io->size ? offset : METAL_BAD_OFFSET);
	}

	return (*io->ops.virt_to_offset)(io, virt);
}
